...
public sealed class GroqLLMClient : BaseLLMClient
{
    ...
    public GroqLLMClient(HttpClient http, ILogger<GroqLLMClient> logger, GroqSettings settings)
        : base(http, logger, settings.ApiKeyEnvVar)
    {
        _settings = settings;
    }

    protected override string GetProviderName() => "Groq";
    ...
    protected override object BuildPayload(RagLLMRequest request)
    {
        ...
        if (request.ConversationHistory != null)
        {
            ...
            foreach (var turn in request.ConversationHistory)
            {
                ...
                {
                    // Generate consistent tool_call_id based on function name and args hash
                ...
                {
                    // Use the SAME tool_call_id from the preceding assistant message
                    messages.Add(new
                    {
                        role = "tool",
                    ...
                    lastToolCallId = null; // Reset for next pair
                }
            }
        }
    ...
    protected override RagLLMResult ParseResponse(JsonDocument doc)
    {
        ...
        throw new InvalidOperationException("Unexpected response format from Groq API");
    }
}
